...
(function() {
  var AStar, AnimatedSearch, Annotations, Dijkstra, Greedy, Grid, GridDiagram, ImmediateNeighbors, InteractiveGrid, JumpPointSuccessors, Map, Node, PathFinder, log, _ref,
  ...
  InteractiveGrid = (function() {
    function InteractiveGrid(element, size) {
      ...
      start = [0, 4];//        => Inline [2,y]              => First [0, 4]            => Second [0, 2]             => Last [2, 4]
      goal = [width -1, 0];//  => Inline [width - 3, y]     => First [width -1, 0]     => Second [width - 1, 3]     => Last [width - 3, 0]
  ...
  AnimatedSearch = (function() {
    ...
    AnimatedSearch.prototype.tick = function() {
      ...
      this.annotations.update(state.open, state.closed, state.paths, state.previous, this.map.start(), this.map.goal(), state.current, null, state.examined);
      return setTimeout(this.tick, this.delay);
    };
  ...
  GridDiagram = (function() {
    function GridDiagram(element, size) {
      var $el, blocked, closed, current, examined, forced, goal, height, open, paths, points, previous, start, width, x, y, _i, _j, _ref;
      if (size == null) {
        size = 20;
      }
      ...
      examined = [];
      if ($el.data('examined')) {
        examined = this.pathsToPoints($el.data('examined'));
      }
      open = [];
      ...
      $el.show();
      this.map.draw();
      this.annotations.update(open, closed, paths, previous, start, goal, current, forced, examined);
    }
  ...
  Grid = (function() {
    ...
    Grid.prototype.appendSVGElements = function() {
      ...
      annotations = translate.append('svg:g').attr('transform', 'scale(1,-1)').attr('class', 'annotations');
  ...
  Annotations = (function() {
    function Annotations(grid) {
      ...
      this.drawPathSection = __bind(this.drawPathSection, this);
    ...
    Annotations.prototype.update = function(open, closed, paths, previous, start, goal, current, forced, examined) {
      this.start = start;
      this.goal = goal;
    ...
    Annotations.prototype.reset = function() {
      this.open = this.closed = this.paths = this.previous = this.forced = this.examined = [];
    ...
    Annotations.prototype.drawPaths = function() {
      var data, pair, _i, _j, _len, _len1, _ref, _ref1;
      data = [];
      _ref = this.paths;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        pair = _ref[_i];
      ...
      this.drawPathSection('.paths', data);
      data = (function() {
        ...
        for (_k = 0, _len2 = _ref2.length; _k < _len2; _k++) {
          pair = _ref2[_k];
          _results.push([pair, 'examined']);
        }
        return _results;
      }).call(this);
      return this.drawPathSection('.extras', data);
    };

    Annotations.prototype.drawPathSection = function(container, data) {
      var paths,
        _this = this;
      paths = this.grid.annotationSelection.select(container).selectAll("line").data(data, function(d, i) {
    ...
    Annotations.prototype.drawSquares = function() {
      ...
      squares = this.grid.annotationSelection.select('.squares').selectAll("rect").data(points, function(d, i) {
        return JSON.stringify([d[0], d[1]]);
      });
      squares.enter().insert('rect', ':first-child').attr('x', function(d, i) {
        return _this.grid.size * d[0];
      }).attr('y', function(d, i) {
        return _this.grid.size * d[1];
    ...
    Annotations.prototype.defineArrowheads = function() {
      ...
      this.defineArrowhead(defs, 'previous');
      return this.defineArrowhead(defs, 'examined');
    };
  ...
  ImmediateNeighbors = (function() {
    ...
    ImmediateNeighbors.prototype.examined = function(node) {
      return [];
    };
  ...
  JumpPointSuccessors = (function(_super) {
    ...
    function JumpPointSuccessors() {
      ...
      this.debugJump = __bind(this.debugJump, this);
      this.jump = __bind(this.jump, this);
      this.examined = __bind(this.examined, this);
      this.successors = __bind(this.successors, this);
      _ref = JumpPointSuccessors.__super__.constructor.apply(this, arguments);
      return _ref;
    }
    ...
    JumpPointSuccessors.prototype.successors = function(node) {
      ...
      jumps = _.map(ns, function(n) {
        var dx, dy, px, py, x, y, _ref1, _ref2;
        _ref1 = node.pos, px = _ref1[0], py = _ref1[1];
        _ref2 = n.pos, x = _ref2[0], y = _ref2[1];
        dx = x - px;
        dy = y - py;
    ...
    JumpPointSuccessors.prototype.examined = function(node) {
      var jumps, ns,
        _this = this;
      ns = this.neighbors(node);
      jumps = _.map(ns, function(n) {
        var dx, dy, js, px, py, value, x, y, _ref1, _ref2, _ref3;
        _ref1 = node.pos, px = _ref1[0], py = _ref1[1];
        _ref2 = n.pos, x = _ref2[0], y = _ref2[1];
        dx = x - px;
        dy = y - py;
        _ref3 = _this.debugJump(node.pos, [dx, dy]), value = _ref3[0], js = _ref3[1];
        return js;
      });
      return jumps = _.flatten(jumps, true);
    };
    ...
    JumpPointSuccessors.prototype.debugJump = function(from, direction) {
      var dx, dy, next, nodes, nx, ny, parent, prev, value, x, xNodes, xValue, y, yNodes, yValue, _ref1, _ref2;
      value = null;
      nodes = [];
      x = from[0], y = from[1];
      dx = direction[0], dy = direction[1];
      ...
      while (this.map.reachable(prev, next)) {
        ...
        if (dx !== 0 && dy !== 0) {
          _ref1 = this.debugJump(next, [dx, 0]), xValue = _ref1[0], xNodes = _ref1[1];
          _ref2 = this.debugJump(next, [0, dy]), yValue = _ref2[0], yNodes = _ref2[1];
      ...
      if (prev !== from) {
        nodes.push(new Node(prev, parent));
      }
      return [null, nodes];
    };
    ...
    JumpPointSuccessors.prototype.neighbors = function(node) {
      var dx, dy, neighbors, ns, px, py, reachable, x, y, _ref1, _ref2,
        _this = this;
      if (node.parent) {
        _ref1 = node.pos, x = _ref1[0], y = _ref1[1];
        _ref2 = node.parent.pos, px = _ref2[0], py = _ref2[1];
        dx = x - px;
        dx = dx > 1 ? 1 : dx < -1 ? -1 : dx;
        dy = y - py;
        dy = dy > 1 ? 1 : dy < -1 ? -1 : dy;
  ...
  PathFinder = (function() {
    ...
    PathFinder.prototype.distance = function(from, to) {
      ...
      _ref1 = from.pos, x1 = _ref1[0], y1 = _ref1[1];
      _ref2 = to.pos, x2 = _ref2[0], y2 = _ref2[1];
      return Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
    };

    PathFinder.prototype.chebyshev = function(from, to) {
      var dx, dy, x1, x2, y1, y2, _ref1, _ref2;
      _ref1 = from.pos, x1 = _ref1[0], y1 = _ref1[1];
      _ref2 = to.pos, x2 = _ref2[0], y2 = _ref2[1];
      dx = Math.abs(x2 - x1);
      dy = Math.abs(y2 - y1);
    ...
    PathFinder.prototype.state = function() {
      var examined, finalPath, nodes, paths, withParents,
        _this = this;
      ...
      examined = _.map(this.examined, function(e) {
        return [e.parent.pos, e.pos];
      });
      finalPath = [];
      if (this.path) {
    ...
    PathFinder.prototype.step = function() {
      var current, existing, neighbor, newG, path, _i, _len, _ref1,
        _this = this;
      if (this.path) {
        return true;
      }
      ...
      _ref1 = this.successors.of(current);
      ...
      this.examined = _.uniq(this.examined.concat(this.successors.examined(current)));
      return null;
    };
